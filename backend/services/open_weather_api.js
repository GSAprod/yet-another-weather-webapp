...
export default class OpenMeteoAPI {
  ...
  async format_response(data, location) {
    ...
    if (weatherCondition === undefined) {
      console.log(
        "Warning: Weather condition with code " +
          data.current.weather_code +
          "is not defined."
      );
      return null;
  ...
  async get_weather(locationParams, optionParams) {
    if (TEST_MODE)
      return [
        200,
        await this.format_response(OpenWeatherTest, locationParams.name),
      ];
    ...
    try {
      ...
      return [
        response.status,
        await this.format_response(response.data, locationParams.name),
      ];
    } catch (error) {
      let errorData = {
        error: true,
  ...
  async append_administrative_region(
    admin_num,
    indexes,
    data,
    formattedResponse
    ...
    Object.keys(similarResults).forEach((key) => {
      ...
      this.append_administrative_region(
        admin_num + 1,
        similarResults[key],
        data,
        formattedResponse
  ...
  async search_geolocations(searchTerm) {
    if (TEST_MODE)
      return [
        200,
        await this.format_geolocation_response(OpenWeatherGeocodingTest),
      ];
    ...
    try {
      ...
      return [
        response.status,
        await this.format_geolocation_response(response.data),
      ];
    } catch (error) {
      let errorData = {
        error: true,
